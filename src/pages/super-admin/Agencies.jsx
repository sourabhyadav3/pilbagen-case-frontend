...
export default function SuperAdminAgencies() {
  ...
  const handleSaveAdd = () => {
    ...
    setActiveModal(null);
    toast(`Agency "${formData.name}" added successfully!`, 'success');
  };

  const handleSaveEdit = () => {
    if (!formData.name || !formData.owner) {
      toast('Please enter valid agency details.', 'error');
      return;
    }
    setAgencies(agencies.map(a => a.id === selectedAgency.id ? { ...a, ...formData } : a));
    setActiveModal(null);
    toast(`Agency "${formData.name}" updated successfully!`, 'success');
  };

  const handleDelete = () => {
    setAgencies(agencies.filter(a => a.id !== selectedAgency.id));
    setActiveModal(null);
    toast(`Agency "${selectedAgency.name}" removed successfully!`, 'success');
  };
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      <PageHeader 
        ...
        subtitle="Manage law firm agencies, subscriptions, offices, and access statuses"
      >
        <button 
      ...
      {isLoading ? (
        <div className="py-20 flex flex-col items-center justify-center gap-4">
          ...
          <p className="text-[12px] text-[#8a94a6] font-800 uppercase tracking-widest">Loading Agencies Data...</p>
        </div>
        ...
        <EmptyState 
          ...
          desc="No law agencies match your filter or search query." 
        />
      ) : (
        <Table headers={['Agency Name', 'Owner Contact', 'Subscription Plan', 'Offices / Users', 'Created Date', 'Status', 'Actions']}>
        ...
        <Modal 
          ...
          footer={
            <>
              ...
              <button onClick={handleSaveAdd} className="px-5 py-2 rounded-xl bg-[#0057c7] text-white text-[13px] font-700">Save Agency</button>
            </>
          }
        >
      ...
      {activeModal === 'edit' && (
        <Modal 
          ...
          footer={
            <>
              <button onClick={() => setActiveModal(null)} className="px-4 py-2 rounded-xl bg-white/5 text-white text-[13px] font-700">Cancel</button>
              <button onClick={handleSaveEdit} className="px-5 py-2 rounded-xl bg-[#0057c7] text-white text-[13px] font-700">Save Changes</button>
            </>
          }
        >
          <div className="space-y-4">
      ...
      {activeModal === 'view' && selectedAgency && (
        <Modal 
          title={`Agency Overview - ${selectedAgency.name}`} 
          onClose={() => setActiveModal(null)}
        ...
        >
          <div className="space-y-4 text-[14px]">
            <div className="grid grid-cols-2 gap-4 p-4 rounded-xl bg-white/5 border border-white/5">
              <div>
                <p className="text-[11px] text-[#8a94a6] uppercase font-700">Agency ID</p>
              ...
              <div>
                <p className="text-[11px] text-[#8a94a6] uppercase font-700">Owner</p>
                <p className="text-white font-600">{selectedAgency.owner}</p>
              </div>
              <div>
                <p className="text-[11px] text-[#8a94a6] uppercase font-700">Email</p>
                <p className="text-white font-600">{selectedAgency.email}</p>
              </div>
              <div>
                <p className="text-[11px] text-[#8a94a6] uppercase font-700">Offices Count</p>
                <p className="text-white font-600">{selectedAgency.officesCount} Offices</p>
              </div>
              <div>
                <p className="text-[11px] text-[#8a94a6] uppercase font-700">Total Users</p>
                <p className="text-white font-600">{selectedAgency.usersCount} Users</p>
              </div>
            </div>
          </div>
        </Modal>
      ...
      {activeModal === 'delete' && selectedAgency && (
        <Modal 
          ...
          footer={
            <>
              <button onClick={() => setActiveModal(null)} className="px-4 py-2 rounded-xl bg-white/5 text-white text-[13px] font-700">Cancel</button>
              <button onClick={handleDelete} className="px-5 py-2 rounded-xl bg-red-600 text-white text-[13px] font-700">Delete Permanently</button>
            </>
          }
        >
          <p className="text-[14px] text-[#b8c2d1]">
            Are you sure you want to delete <strong className="text-white">{selectedAgency.name}</strong>? This action cannot be undone and will remove all associated offices and users.
          </p>
        </Modal>
      )}
    </div>
